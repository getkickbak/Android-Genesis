...
public class ConnectPlugin extends Plugin {
		...
		if (action.equals("init")) {
			...
			if (facebook != null) {
				...
				Runnable runnable = new Runnable() {
					public void run() {
						me.facebook.authorize(cordova.getActivity(),
			...
			if (facebook != null) {
				try {
					...
					SharedPreferences prefs = PreferenceManager
							.getDefaultSharedPreferences(this.cordova
									.getActivity());
			...
			if (facebook != null) {
				...
				pr = new PluginResult(PluginResult.Status.ERROR,
						"Must call init before getLoginStatus.");
			}
		}
			...
			if (facebook != null) {
				...
				Runnable runnable = new Runnable() {
					public void run() {
						me.facebook.dialog(me.cordova.getActivity(), me.method,
								me.paramBundle, new UIDialogListener(me));
					};
				};
				cordova.getActivity().runOnUiThread(runnable);
				// this.ctx.runOnUiThread(runnable);
	...
	public JSONObject getResponse() {
		...
		if (facebook.isSessionValid()) {
			long expiresTimeInterval = facebook.getAccessExpires()
					- System.currentTimeMillis();
			long expiresIn = (expiresTimeInterval > 0) ? expiresTimeInterval
					: 0;
			response = "{" + "\"status\": \"connected\","
					+ "\"authResponse\": {" + "\"accessToken\": \""
					+ facebook.getAccessToken() + "\"," + "\"expiresIn\": \""
	...
	class UIDialogListener implements DialogListener {
		...
		public void onError(DialogError e) {
			...
			this.fba.error("Dialog error: " + e.getMessage(),
					this.fba.callbackId);
		}
	...
	class AuthorizeListener implements DialogListener {
		...
		public void onComplete(Bundle values) {
			...
			SharedPreferences prefs = PreferenceManager
					.getDefaultSharedPreferences(this.fba.cordova.getActivity());
